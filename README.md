...
- **The container stack was never run on the development machine** — Docker is not installed there. It is verified in CI instead: the `stack` job brings the full compose stack up, waits on every healthcheck, then asserts the API contract, that the dashboard is served, that the Timescale hypertable and continuous aggregate exist, and that the ingest service is producing to the Kafka topic. Two real bugs surfaced that way on the first runs — a missing `libgomp1` for LightGBM, and an nginx healthcheck resolving `localhost` to `::1` with no IPv6 listener.
